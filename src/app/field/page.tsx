...
export default async function FieldDashboardPage({ searchParams }: { searchParams: Promise<{ month?: string, year?: string }> | { month?: string, year?: string } }) {
  ...
  const data = await getMonthlyData(month, year, 'FIELD')

  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-2 sm:p-4 bg-slate-50 ">
      <Dashboard initialData={data} initialMonth={month} initialYear={year} basePath="/field" title="Field Attendance Dashboard" />
    </main>
  )
}
